...
export function renderChart(container, validSessions) {
  ...
  const minData = Math.min(...kgValues);
  const maxData = Math.max(...kgValues);
  ...
  const yFor = (kg) => PAD_TOP + chartH * (1 - (kg - minDisplay) / (maxDisplay - minDisplay));

  const gridLines = ticks.map((t) => {
    const y = yFor(t);
    return `<line x1="${PAD_LEFT}" y1="${y}" x2="${WIDTH - PAD_RIGHT}" y2="${y}" stroke="var(--chart-grid)" stroke-width="1" />
            <text x="${PAD_LEFT - 6}" y="${y + 4}" text-anchor="end" font-size="9" fill="var(--text-muted)">${t} kg</text>`;
  }).join("");
  ...
  points.forEach((p, i) => {
    ...
    repLabels += `<text x="${x}" y="${y - 10}" text-anchor="middle" font-size="10" fill="var(--chart-label)" font-weight="700">${p.r}</text>`;
    if (shouldLabel(i)) {
